...
int main(){
    ...
    big k1 = mirvar(0), k2 = mirvar(0);
    epoint* kk1 = epoint_init();
    epoint* kk2 = epoint_init();
    irand(time(NULL));
	...
	bigrand(n, k2);
        cotnum(k1, stdout);
	cotnum(k2, stdout);
	ecurve_mult(k1, G, kk1);
	ecurve_mult(k2, G, kk2);
        tstart=clock();
        ecurve_add(kk1, kk2);
        tend=clock();
        duration += (double)(tend-tstart)/CLOCKS_PER_SEC;
    }
    printf("1000 times ecurve_add duration: %lf\n", duration);
    mirkill(a);
    mirkill(b);
    mirkill(p);
    mirkill(Gx);
